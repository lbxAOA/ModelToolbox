...
// ===================== MCP additions: design rules + DRC =====================

function MCPSplit(S: String; Delim: String): TStringList;
var
    P: Integer;
begin
    ...
    begin
        ...
        S := Copy(S, P + Length(Delim), Length(S));
    end;
    Result.Add(S);
end;

function MCPApplyRuleParam(Rule: IPCB_Rule; Key: String; Value: String): String;
var
    V: Integer;
begin
    Result := '';
    ...
    try
        if Key = 'scope1' then
            Rule.Scope1Expression := Value
        else if Key = 'scope2' then
            Rule.Scope2Expression := Value
        else if Key = 'enabled' then
            Rule.DRCEnabled := (Value = '1')
        else
        begin
            V := MilsToCoord(StrToInt(Value));
            if Key = 'gap' then Rule.Gap := V
            else if Key = 'minwidth' then Rule.MinWidth := V
            else if Key = 'maxwidth' then Rule.MaxWidth := V
            else if Key = 'favoredwidth' then Rule.FavoredWidth := V
            else if Key = 'prefviawidth' then Rule.PreferedWidth := V
            else if Key = 'minlimit' then Rule.MinLimit := V
            else if Key = 'maxlimit' then Rule.MaxLimit := V
            else if Key = 'minhole' then Rule.MinHoleWidth := V
            else if Key = 'maxhole' then Rule.MaxHoleWidth := V
            else if Key = 'prefhole' then Rule.PreferedHoleWidth := V
            else Result := 'unknown key: ' + Key;
        end;
    except
        Result := 'failed setting ' + Key + '=' + Value;
    end;
end;

function ExecuteUpdateDesignRules(RequestData: TStringList): String;
var
    Board: IPCB_Board;
    Spec, EditStr, Action, Target, ParamsStr, Pair, Key, Value, Err, EditLog: String;
    i, j, k, P, MatchCount: Integer;
    Rule: IPCB_Rule;
    Iter: IPCB_BoardIterator;
    Edits, Fields, Params, ResultArr, RP, OutputLines, MatchedRules: TStringList;
begin
    Board := PCBServer.GetCurrentPCBBoard;
    if Board = nil then
    begin
        Result := '{"success": false, "error": "No PCB document active"}';
        Exit;
    end;

    Spec := '';
    for i := 0 to RequestData.Count - 1 do
        if Pos('"rules_spec"', RequestData[i]) > 0 then
        begin
            P := Pos(':', RequestData[i]);
            Spec := TrimJSON(Copy(RequestData[i], P + 1, Length(RequestData[i])));
        end;
    if Spec = '' then
    begin
        Result := '{"success": false, "error": "No rules_spec provided"}';
        Exit;
    end;

    ResultArr := TStringList.Create;
    Edits := MCPSplit(Spec, '~');
    try
        for i := 0 to Edits.Count - 1 do
        begin
            EditStr := Trim(Edits[i]);
            if EditStr = '' then Continue;
            Fields := MCPSplit(EditStr, '|');
            try
                Action := UpperCase(Trim(Fields[0]));
                Target := '';
                ParamsStr := '';
                if Fields.Count > 1 then Target := Trim(Fields[1]);
                if Fields.Count > 2 then ParamsStr := Trim(Fields[2]);
                EditLog := '';
                MatchCount := 0;
                ...
                begin
                    ...
                    try
                        ...
                        begin
                            ...
                            P := Pos('=', Pair);
                            Key := Copy(Pair, 1, P - 1);
                            Value := Copy(Pair, P + 1, Length(Pair));
                            if LowerCase(Key) = 'kind' then Continue;
                            Err := MCPApplyRuleParam(Rule, Key, Value);
                            if Err <> '' then EditLog := EditLog + Err + '; ';
                        end;
                    finally
                        Params.Free;
                    end;
                ...
                begin
                    // find matching rules: KIND:<short descriptor> or exact name
                    ...
                    while Rule <> nil do
                    begin
                        if ((Pos('KIND:', Target) = 1) and (Rule.GetState_ShortDescriptorString = Copy(Target, 6, Length(Target)))) or
                           ((Pos('KIND:', Target) = 0) and (Rule.Name = Target)) then
                        begin
                            MatchCount := MatchCount + 1;
                            if Action = 'DISABLE' then Rule.DRCEnabled := False
                            else if Action = 'ENABLE' then Rule.DRCEnabled := True
                ...
                try
                    AddJSONProperty(RP, 'edit', Action + ' ' + Target);
                    AddJSONInteger(RP, 'matched', MatchCount);
                    AddJSONProperty(RP, 'errors', EditLog);
                    ResultArr.Add(BuildJSONObject(RP, 1));
                finally
                    RP.Free;
                end;
            finally
        ...
        OutputLines := TStringList.Create;
        try
            OutputLines.Text := BuildJSONArray(ResultArr);
            Result := OutputLines.Text;
        finally
            OutputLines.Free;
        end;
...
function ExecuteRunDRC(RequestData: TStringList): String;
var
    Board: IPCB_Board;
    RP, OutputLines: TStringList;
    RunMethod: String;
begin
    Board := PCBServer.GetCurrentPCBBoard;
    if Board = nil then
    ...
    RP := TStringList.Create;
    try
        AddJSONBoolean(RP, 'success', RunMethod <> 'failed');
        AddJSONProperty(RP, 'method', RunMethod);
        OutputLines := TStringList.Create;
        try
            OutputLines.Text := BuildJSONObject(RP);
            Result := OutputLines.Text;
        finally
            OutputLines.Free;
        end;
