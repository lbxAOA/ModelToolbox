...
// ===================== MCP addition: add text annotations to SchDoc =====================

function MCPDecodeText(S: String): String;
var
    i: Integer;
    HexStr: String;
begin
    ...
    while i <= Length(S) do
    begin
        if (Copy(S, i, 2) = '%u') and (i + 5 <= Length(S)) then
        begin
            HexStr := Copy(S, i + 2, 4);
...
function ExecuteAddSchematicText(RequestData: TStringList): String;
var
    SchDoc: ISch_Document;
    Lbl: ISch_Label;
    Spec, ItemStr, Txt: String;
    i, P, XMils, YMils, Added: Integer;
    Items, Fields, RP, OutputLines: TStringList;
begin
    if SchServer = nil then
    begin
        Result := '{"success": false, "error": "Schematic server not available"}';
        Exit;
    end;
    SchDoc := SchServer.GetCurrentSchDocument;
    if SchDoc = nil then
    begin
        Result := '{"success": false, "error": "No schematic document active"}';
        Exit;
    end;

    Spec := '';
    for i := 0 to RequestData.Count - 1 do
        if Pos('"texts_spec"', RequestData[i]) > 0 then
        begin
            P := Pos(':', RequestData[i]);
    ...
    if Spec = '' then
    begin
        Result := '{"success": false, "error": "No texts_spec provided"}';
        Exit;
    end;
    ...
    try
        ...
        begin
            ItemStr := Trim(Items[i]);
            if ItemStr = '' then Continue;
            Fields := MCPSplit(ItemStr, '|');
            try
                if Fields.Count < 3 then Continue;
                XMils := StrToInt(Trim(Fields[0]));
                YMils := StrToInt(Trim(Fields[1]));
                Txt := MCPDecodeText(Trim(Fields[2]));

                Lbl := SchServer.SchObjectFactory(eLabel, eCreate_Default);
                if Lbl = nil then Continue;
                Lbl.Location := Point(MilsToCoord(XMils), MilsToCoord(YMils));
                Lbl.Text := Txt;
                SchDoc.RegisterSchObjectInContainer(Lbl);
                SchServer.RobotManager.SendMessage(SchDoc.I_ObjectAddress, c_BroadCast,
                    SCHM_PrimitiveRegistration, Lbl.I_ObjectAddress);
                Added := Added + 1;
            finally
                Fields.Free;
            end;
    ...
    try
        AddJSONBoolean(RP, 'success', True);
        AddJSONInteger(RP, 'texts_added', Added);
        OutputLines := TStringList.Create;
        try
