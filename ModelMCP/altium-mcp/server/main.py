...
mcp = FastMCP("AltiumMCP", instructions="Altium integration through the Model Context Protocol")
...
@mcp.tool()
async def update_design_rules(ctx: Context, edits: list) -> str:
    """
    Modify, create, enable or disable PCB design rules in the active board.

    Args:
        edits (list): list of edit dicts:
          {"action": "modify"|"create"|"disable"|"enable",
           "target": "KIND:<rule kind>" (e.g. "KIND:Width Constraint") or exact rule name;
                     for create: the new rule's name,
           "params": {"gap":6, "minwidth":6, "maxwidth":120, "prefwidth":10,
                      "minlimit":10, "maxlimit":140, "minhole":12, "maxhole":32,
                      "prefhole":28, "scope1":"InNet('VS') OR InNet('PWR')",
                      "scope2":"All", "enabled":1}}
          All dimension values are integers in mils. Strings must not contain
          double quotes, commas, '~', '|' or ';' characters.

    Returns:
        str: JSON array with per-edit result (matched count, errors)
    """
    parts = []
    for e in edits:
        action = str(e.get("action", "modify")).upper()
        target = str(e.get("target", ""))
        params = e.get("params", {}) or {}
        pstr = ";".join(f"{k}={v}" for k, v in params.items())
        parts.append(f"{action}|{target}|{pstr}")
    spec = "~".join(parts)
    for bad in ('"', ','):
        if bad in spec:
            return json.dumps({"success": False, "error": f"illegal character {bad!r} in spec"})
    logger.info(f"Updating design rules: {spec[:200]}")
    response = await altium_bridge.execute_command("update_design_rules", {"rules_spec": spec})
    if not response.get("success", False):
...
async def run_drc(ctx: Context) -> str:
    """
    Run the batch Design Rule Check (DRC) on the active PCB document.
    Uses the board's current DRC settings; generates the standard DRC report.

    Returns:
        str: JSON object {success, method, violations_on_board}
    """
...
@mcp.tool()
async def add_schematic_text(ctx: Context, items: list) -> str:
    """
    Add text annotations (labels) to the active schematic document.

    Args:
        items (list): list of dicts: {"text": "注释内容", "x": 1000, "y": 2000}
                      x/y are in mils from the sheet origin (bottom-left).

    Returns:
        str: JSON object {success, texts_added}
    """
    def enc(t: str) -> str:
        out = []
        for ch in str(t):
            if ch.isascii() and (ch.isalnum() or ch in " _.:()/+-"):
                ...
                hx = ch.encode("utf-16-be").hex().upper()
                for i in range(0, len(hx), 4):
                    out.append("%u" + hx[i:i+4])
        return "".join(out)

    parts = []
    for it in items:
        x = int(it.get("x", 1000)); y = int(it.get("y", 1000))
        parts.append(f"{x}|{y}|{enc(it.get('text',''))}")
    spec = "~".join(parts)
    logger.info(f"Adding {len(parts)} schematic texts")
    response = await altium_bridge.execute_command("add_schematic_text", {"texts_spec": spec})
    if not response.get("success", False):
        return json.dumps({"success": False, "error": response.get("error", "Unknown error")})
    return json.dumps(response.get("result", {}), indent=2)
